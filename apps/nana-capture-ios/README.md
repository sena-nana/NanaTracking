...
- On iOS with ARKit, `ARKitCaptureSessionController` drives a real face-tracking `ARSession`, applies
  Studio start/pause/stop/retake/end commands, and hands frames to one bounded asynchronous
  latest-only worker. `ARKitCapturePipeline` extracts RGB, raw blendshapes, face/eye transforms,
  geometry, camera intrinsics, tracking state, and front-camera `capturedDepthData`. Depth keeps its
  independent timestamp and explicit unavailable confidence instead of inheriting RGB timing or an
  invented score.
- Raw frame URIs are generated by the same canonical function as durable chunk descriptors; the
  JSON cannot name a file that the recorder did not write.
...
storage, transport, and schema paths build and self-test here, but type-checking/signing the iOS
ARKit branch and proving TrueDepth device lifecycle/background behavior still require full Xcode
and a supported device. The cross-platform Studio backend/UI is implemented separately; its Windows
and RTX workflow remains Windows-side acceptance work.
